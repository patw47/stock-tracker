...
`,options:{}},type:`n8n-nodes-base.gmail`,typeVersion:2.1,position:[1264,-192],id:`4a23b2b5-21f7-4a63-a1ab-9f18d6ce2e6a`,name:`Send output via email using Gmail`,webhookId:`65c7b462-bb4a-400c-a556-ef408efcd208`,notesInFlow:!0,notes:`Double-click here to connect!`}],pinData:{},connections:{"Get city latitude and longitude":{main:[[{node:`Limit`,type:`main`,index:0}]]},"Get weather":{main:[[{node:`Get today's high and low`,type:`main`,index:0}]]},"Set your location":{main:[[{node:`Get city latitude and longitude`,type:`main`,index:0}]]},Merge:{main:[[{node:`Aggregate`,type:`main`,index:0}]]},Aggregate:{main:[[{node:`Select output for the email`,type:`main`,index:0}]]},"Get today's high and low":{main:[[{node:`Merge`,type:`main`,index:2}]]},Limit:{main:[[{node:`Get weather`,type:`main`,index:0}]]},"1. Click 'Execute workflow’":{main:[[{node:`Set current day and month`,type:`main`,index:0},{node:`Set your location`,type:`main`,index:0},{node:`Set exchange currency`,type:`main`,index:0}]]},"Set current day and month":{main:[[{node:`Get a historical fact`,type:`main`,index:0}]]},"Set exchange currency":{main:[[{node:`Get exchange rates`,type:`main`,index:0}]]},"Get a historical fact":{main:[[{node:`Merge`,type:`main`,index:0}]]},"Get exchange rates":{main:[[{node:`Merge`,type:`main`,index:1}]]},"Select output for the email":{main:[[]]}}},h=`N8N_READY_TO_RUN_WORKFLOWS_DISMISSED`,g=s(c.EXPERIMENT_READY_TO_RUN_WORKFLOWS,()=>{let s=i(),c=t(),g=u(),_=a(),v=r(),y=l(),b=e(()=>[o.variantReadyToRun,o.variantReadyToRun2,o.variantReadyToRun3].includes(v.getVariant(o.name)?.toString()??``)&&y.userIsTrialing),x=n(h,!1),S=e(()=>x.value),C=()=>{x.value=!0},w=e=>{s.track(`User created ready to run workflows`,{source:e})},T=()=>{s.track(`User dismissed ready to run workflows callout`)},E=e=>{s.track(`User opened ready to run workflow`,{template:e})},D=(e,t)=>{s.track(`User executed ready to run workflow`,{template:e,status:t})};function O(){switch(v.getVariant(o.name)){case o.variantReadyToRun:return c.baseText(`workflows.readyToRunWorkflows.card`);case o.variantReadyToRun2:return c.baseText(`workflows.readyToRunWorkflows.card2`);case o.variantReadyToRun3:return c.baseText(`workflows.readyToRunWorkflows.card3`);default:return``}}function k(){switch(v.getVariant(o.name)){case o.variantReadyToRun:return c.baseText(`workflows.readyToRunWorkflows.callout`);case o.variantReadyToRun2:return c.baseText(`workflows.readyToRunWorkflows.callout2`);case o.variantReadyToRun3:return c.baseText(`workflows.readyToRunWorkflows.callout3`);default:return``}}return{isFeatureEnabled:b,isCalloutDismissed:S,createWorkflows:async(e,t)=>{let n=await g.createFolder(c.baseText(`workflows.readyToRunWorkflows.folder.name`),e,t),r={...d,parentFolderId:n.id},i={...f,parentFolderId:n.id},a={...p,parentFolderId:n.id},o={...m,parentFolderId:n.id};return await _.createNewWorkflow(o),await _.createNewWorkflow(a),await _.createNewWorkflow(i),await _.createNewWorkflow(r),C(),n},dismissCallout:C,trackCreateWorkflows:w,trackDismissCallout:T,trackOpenWorkflow:E,trackExecuteWorkflow:D,getCardText:O,getCalloutText:k}});export{g as t};
//# sourceMappingURL=readyToRunWorkflows.store-DtiUg3Dc.js.map